# When2Hangout｜十年之期已到！恭迎......

一个适合分享到微信群的聚会时间收集网页。第一版只服务于一次固定活动：收集朋友们在 `2026-07-27` 至 `2026-08-30` 之间每周五、周六、周日午餐和晚餐的可用时间。
...
- 固定显示五个自然周的周标题，每周只展示星期五、星期六、星期日。
- 每天只有两个时段：午餐、晚餐。
...
- 五周表格之后提供独立的“本次无法参与”选项，适合以上时间都不合适或这段时间在外地的朋友。
...
每次 `submit-availability` 成功提交/更新，或 `clear-submission` 成功清空后，后端会尝试把最新表格发到 `OWNER_EMAIL`。邮件会说明哪个名字新增、修改或清空了日期填写，并列出新增/取消的具体午晚时段；表格按提交时间升序排列。邮件失败不会阻止用户提交。
